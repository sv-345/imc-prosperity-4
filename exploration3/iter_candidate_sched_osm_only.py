...
# SCHEDULE candidate: deterministic taker schedule from 3-day training.
# ADDITIVE widen: on scheduled BUY-taker ticks, push ask UP by this many
# ticks beyond the normal ba-1. Mirror for bid.
SCHED_WIDEN = 2
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0, ts=0):
        ...
        orders = []
        opos = pos
        # SCHEDULE: look up if this tick has a deterministic taker event
        sched = OSM_SCHEDULE.get(ts, [])
        sched_buy_qty = sum(q for dr, q in sched if dr == "BUY")
        sched_sell_qty = sum(q for dr, q in sched if dr == "SELL")
        ...
        if bb is not None and ba is not None:
            ...
            # SCHEDULE: widen our ask UP (or bid DOWN) additively on scheduled events.
            if sched_buy_qty > 0:
                ask_pj = ask_pj + SCHED_WIDEN
            if sched_sell_qty > 0:
                bid_pj = bid_pj - SCHED_WIDEN
            bid_edge = fv - edge
            ask_edge = fv + edge
        ...
        if pos < 70:
            ...
            if buy_cap > 0 and bb is not None:
                ...
                if defend_bid:
                    ...
                    buy_cap = int(round(buy_cap * ADVERSE_SHRINK))
                # SCHEDULE: if scheduled sell-taker incoming, lower our bid
                if sched_sell_qty > 0:
                    bid_price = bid_price - SCHED_WIDEN
                if buy_cap > 0:
            ...
            if take_sig >= NET_TAKE_THRESH_DEF:
                ...
                base_sell = 8
            # SCHEDULE: if buy-taker incoming, sell more
            if sched_buy_qty > 0:
                base_sell = max(base_sell, sched_buy_qty)
            sell_qty = min(base_sell, limit + pos)
            if sell_qty > 0:
                ask_price = max(ba - 1, fv_int + 1) if ba else fv_int + 7
                # SCHEDULE: on buy-taker tick, push ask up
                if sched_buy_qty > 0:
                    ask_price = ask_price + SCHED_WIDEN
