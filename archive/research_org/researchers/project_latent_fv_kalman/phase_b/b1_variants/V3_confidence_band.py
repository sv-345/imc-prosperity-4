"""V3 — Confidence-band quoting (OSM).

Same as V1 (full Kalman FV feed) PLUS: use the posterior variance P as
a confidence signal to scale the OSM outer-wall quote edge.

When P is large (filter uncertain, early session or after a shock) we
widen the outer wall (edge > 20). When P is at steady-state P_inf
(filter converged) we tighten the outer wall toward edge=20. We do
NOT change the inner quote — that stays at bb+1 / ba-1 / fv+-1 per
iter23 — because the inner quote is already at the tightest legal
position.

Scaling: ``factor = sqrt(P / P_inf)``. Clamp to [0.8, 2.0] to bound
behaviour during early-session transients.  edge = round(20 * factor).

PEP uses Kalman FV same as V1 (keeping it consistent; V3 is a pure
OSM-edge experiment on top of the V1 FV feed).
"""
...
# V3-specific knobs (will be sensitised in V3b/V3c variants if useful)
V3_EDGE_FACTOR_MIN = 0.8
V3_EDGE_FACTOR_MAX = 2.0
V3_BASE_EDGE = 20


def _v3_edge(P: float) -> int:
    factor = math.sqrt(max(P, 1e-6) / OSM_P_INF)
    factor = max(V3_EDGE_FACTOR_MIN, min(V3_EDGE_FACTOR_MAX, factor))
    return int(round(V3_BASE_EDGE * factor))
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if osm in state.order_depths and osm_fv is not None:
            result[osm] = self._trade_osmium(
                ...
                take_sig.get(osm, 0), osm_fv, osm_P or OSM_P_INF,
            )
    ...
    def _trade_osmium(self, product, d, pos, take_sig, fv_float, P):
        orders = []
        opos = pos
        ...
        edge = _v3_edge(P)  # <-- V3 change: P-scaled outer wall
