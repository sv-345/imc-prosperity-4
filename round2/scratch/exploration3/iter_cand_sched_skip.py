...
# SKIP candidate: withdraw our quote on scheduled adverse ticks.
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0, ts=0):
        ...
        skip_ask = any(dr == "BUY" for dr, q in sched)   # buy-taker coming, mid rises, selling at F+7 is stale
        skip_bid = any(dr == "SELL" for dr, q in sched)  # sell-taker coming, mid falls, buying at F-5 is stale
        orders = []
        opos = pos
        ...
        if bb is not None and ba is not None:
            ...
            if bid_pj <= bid_edge:
                ...
                if bpj > 0 and not skip_bid:   # SKIP: withdraw inner bid on scheduled SELL-taker tick
            ...
            if ask_pj >= ask_edge:
                ...
                if spj > 0 and not skip_ask:   # SKIP: withdraw inner ask on scheduled BUY-taker tick
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0, ts=0):
        ...
        skip_ask = any(dr == "BUY" for dr, q in sched)
        skip_bid = any(dr == "SELL" for dr, q in sched)
        orders = []
        bb = self._bb(d)
        ba = self._ba(d)
        ...
        if pos < 70:
            ...
            buy_cap = limit - pos
            if buy_cap > 0 and bb is not None and not skip_bid:
            ...
            if take_sig >= NET_TAKE_THRESH_DEF:
                ...
                base_sell = 8
            sell_qty = min(base_sell, limit + pos) if not skip_ask else 0
            if sell_qty > 0:
