...
# SIZEUP: post larger quote on aligned scheduled ticks.
SCHED_SIZE_BOOST = 10
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0, ts=0):
        ...
        boost_sell = sum(q for dr, q in sched if dr == "BUY")   # buy-taker coming, we sell more
        boost_buy = sum(q for dr, q in sched if dr == "SELL")
        orders = []
        opos = pos
        ...
        if bb is not None and ba is not None:
            ...
            mi = 15
            mi_bid = max(mi, boost_buy + mi) if boost_buy > 0 else mi   # SIZEUP
            mi_ask = max(mi, boost_sell + mi) if boost_sell > 0 else mi
            if bid_pj <= bid_edge:
                if buy_cap > 0:
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0, ts=0):
        ...
        boost_sell = sum(q for dr, q in sched if dr == "BUY")
        orders = []
        bb = self._bb(d)
        ba = self._ba(d)
        ...
        if pos < 70:
            ...
            if take_sig >= NET_TAKE_THRESH_DEF:
                ...
                base_sell = 8
            if boost_sell > 0:
                base_sell = max(base_sell, boost_sell + 5)   # SIZEUP on scheduled BUY
