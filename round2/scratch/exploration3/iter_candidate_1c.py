"""Iter candidate 1 — iter23 + wall-volume-imbalance defensive skew.

Signal: (outermost-bid-vol − outermost-ask-vol) predicts next-tick
detrended-mid move monotonically. |t| up to 4.4 at |imb|>=15, p<0.001.
Layered ON TOP of the existing take_sig defensive widen; both fire
simultaneously when both point the same way.

Change: add imb_up / imb_down flags computed alongside defend_ask/bid.
When imb>=+IMB_THRESH (up-pressure), widen ASK further (stacks with
take_sig). When imb<=-IMB_THRESH (down-pressure), widen BID further.

Applied to OSM only. PEP already captures drift from deterministic FV
and the imb signal there is confounded by drift.

Original iter23 docstring:
---
iter23 — iter22 + signal-gated PEP recycle size.
...
# candidate-1c: OFFENSIVE — imb signal drives bid/ask TIGHTENING on
# predicted direction (opposite of defensive).
# imb>0 → mid will rise → TIGHTEN bid (bid_pj += 1) to accumulate long.
# imb<0 → mid will fall → TIGHTEN ask (ask_pj -= 1) to accumulate short.
IMB_THRESH = 10
IMB_WIDEN = -1   # negative value = tighten by 1 on predicted side
IMB_SHRINK = 1.0  # no shrink
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        # candidate-1: wall-volume-imbalance signal.
        # outermost bid/ask volumes. imb>0 predicts up; imb<0 predicts down.
        imb = 0
        if d.buy_orders and d.sell_orders:
            bid_wall_px = min(d.buy_orders)   # lowest bid price = outer wall
            ask_wall_px = max(d.sell_orders)  # highest ask price = outer wall
            bid_wall_vol = d.buy_orders[bid_wall_px]
            ask_wall_vol = -d.sell_orders[ask_wall_px]
            imb = bid_wall_vol - ask_wall_vol
        imb_up = imb >= IMB_THRESH
        imb_down = imb <= -IMB_THRESH
        ...
        if bb is not None and ba is not None:
            ...
            # candidate-1c: offensive skew — imb>0 tightens bid (to accumulate long before up-move);
            # imb<0 tightens ask (to accumulate short before down-move).
            # IMB_WIDEN is NEGATIVE (=-1) here, so bid_pj -= -1 = bid_pj + 1 (tighter),
            # ask_pj += -1 = ask_pj - 1 (tighter). Mirror effect vs 1a.
            if imb_up:
                bid_pj = bid_pj - IMB_WIDEN  # bid_pj + 1 (tighter bid)
            if imb_down:
                ask_pj = ask_pj + IMB_WIDEN  # ask_pj - 1 (tighter ask)
            bid_edge = fv - edge
            ask_edge = fv + edge
