...
class Trader:
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0):
        ...
        if pos < 70:
            ...
            # candidate-3: pos-scaled recycle sell — sell more as pos approaches limit
            if take_sig >= NET_TAKE_THRESH_DEF:
                base_sell = 0  # skip adverse
            elif take_sig <= -NET_TAKE_THRESH_DEF:
                base_sell = 15
            else:
                # at pos=70 base=8; at pos=100 base=18
                base_sell = 8 + max(0, (pos - 70) // 3)
