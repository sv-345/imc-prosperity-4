...
# candidate-1: wall-imbalance defensive skew
IMB_THRESH = 8
IMB_WIDEN = 2  # extra ticks on top of ADVERSE_WIDEN when stacking
IMB_SHRINK = 0.5
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        if bb is not None and ba is not None:
            ...
            # candidate-1: imb-based defensive widen (stacks additively)
            if imb_up:
                ask_pj = ask_pj + IMB_WIDEN
                sell_cap = int(round(sell_cap * IMB_SHRINK))
            if imb_down:
                bid_pj = bid_pj - IMB_WIDEN
                buy_cap = int(round(buy_cap * IMB_SHRINK))
            bid_edge = fv - edge
            ask_edge = fv + edge
