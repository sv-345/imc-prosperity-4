"""Iter candidate 2 — iter23 + L1-imbalance-conditional gate extension.

Signal: L1 inner-quote vol imbalance (bid_vol_L1 − ask_vol_L1) predicts
next-tick mid by 1-3 ticks per event with t-stats 10-25 (OSM and PEP).
Mechanism: thin ask L1 gets eaten first; ask L1 walks up to the next
level; mid rises.

Change: when L1 imb>=THRESH, extend OSM take-ask gate from OSM_FV to
OSM_FV + IMB_EXTEND. Mirror on bid side.
...
# candidate-2: L1-imbalance-conditional gate extension
L1_IMB_THRESH = 8
L1_IMB_EXTEND = 4  # extend gate by this many ticks when imb signals
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        # candidate-2: L1 volume imbalance (bid_vol_L1 - ask_vol_L1)
        # imb_up (bid side deep, ask thin) → mid about to rise → extend buy gate
        # imb_down (ask deep, bid thin) → mid about to fall → extend sell gate
        bid_vol_L1 = d.buy_orders[bb] if bb is not None else 0
        ask_vol_L1 = -d.sell_orders[ba] if ba is not None else 0
        l1_imb = bid_vol_L1 - ask_vol_L1
        ask_gate = fv + (L1_IMB_EXTEND if l1_imb >= L1_IMB_THRESH else 0)
        bid_gate = fv - (L1_IMB_EXTEND if l1_imb <= -L1_IMB_THRESH else 0)
