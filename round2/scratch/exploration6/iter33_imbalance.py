...
# iter33: book-imbalance defensive signal.
# Toxic SELL (mid rises after) avg imb = +0.284 (bid-heavy). Toxic BUY avg = -0.192.
# Tunable via env vars.
import os as _os
IMB_THRESH = float(_os.environ.get("IMB_THRESH", "0.15"))
IMB_WIDEN = int(_os.environ.get("IMB_WIDEN", "3"))
IMB_SHRINK = float(_os.environ.get("IMB_SHRINK", "0.3"))

def _book_imbalance(d):
    bv = sum(d.buy_orders.values()) if d.buy_orders else 0
    av = sum(-v for v in d.sell_orders.values()) if d.sell_orders else 0
    total = bv + av
...
class Trader:
    ...
    def run(self, state):
        ...
        osm_k = OSM_PHASE3_K_INSIDE if _in_window(state.timestamp) else OSM_PHASE3_K_OUTSIDE
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0, osm_k=1):
        ...
        orders = []
        opos = pos
        # iter22: defensive bot-take signal (tape-based).
        defend_ask = take_sig >= NET_TAKE_THRESH_DEF
        defend_bid = take_sig <= -NET_TAKE_THRESH_DEF
        # iter33: book-imbalance signal (book-state-based).
        # imb > +thresh → bid-heavy → ask toxic → defend ask.
        # imb < -thresh → ask-heavy → bid toxic → defend bid.
        imb = _book_imbalance(d)
        if imb > IMB_THRESH:
            defend_ask = True
        if imb < -IMB_THRESH:
            defend_bid = True
