...
import os as _os
IMB_CAP_THRESH = float(_os.environ.get("IMB_CAP_THRESH", "0.40"))
IMB_CAP_SHRINK = float(_os.environ.get("IMB_CAP_SHRINK", "0.50"))
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0, osm_k=1):
        ...
        # iter34: book-imbalance cap shrink. Don't move price; cut exposure on toxic side.
        imb = _book_imbalance(d)
        if imb > IMB_CAP_THRESH:
            sell_cap = int(round(sell_cap * IMB_CAP_SHRINK))
        elif imb < -IMB_CAP_THRESH:
            buy_cap = int(round(buy_cap * IMB_CAP_SHRINK))
