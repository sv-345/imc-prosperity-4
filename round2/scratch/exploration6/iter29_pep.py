...
# iter29: PEP-specific tighter sensitivity for recycle sell-skip.
# Default threshold was 3; lowering it makes recycle skip fire on milder uptrends.
import os as _os
PEP_SKIP_THRESH = int(_os.environ.get("PEP_SKIP_THRESH", "1"))
PEP_AGG_THRESH = int(_os.environ.get("PEP_AGG_THRESH", "3"))   # keep high for safety
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        if bb is not None and ba is not None:
            ...
            bid_pj = min(bb + 1, int(dynamic_fv) - OSM_PHASE3_K)
            ask_pj = max(ba - 1, int(dynamic_fv) + OSM_PHASE3_K)
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0):
        ...
        if pos < 70:
            ...
            if buy_cap > 0 and bb is not None:
                bid_price = min(bb + 1, fv_int - PEP_PHASE3_K)
                if defend_bid:
                    bid_price -= ADVERSE_WIDEN
                    buy_cap = int(round(buy_cap * ADVERSE_SHRINK))
                if buy_cap > 0:
                    orders.append(Order(product, bid_price, buy_cap))
            # iter29: asymmetric PEP thresholds.
            # Lower PEP_SKIP_THRESH → more eager to skip recycle on uptrends (ride the drift).
            # Keep PEP_AGG_THRESH high → only aggressive-sell on strong down-pressure.
            if take_sig >= PEP_SKIP_THRESH:
                base_sell = 0
            elif take_sig <= -PEP_AGG_THRESH:
                base_sell = 15
            else:
                base_sell = 8
            sell_qty = min(base_sell, limit + pos)
            if sell_qty > 0:
                ask_price = max(ba - 1, fv_int + PEP_PHASE3_K) if ba else fv_int + 7
