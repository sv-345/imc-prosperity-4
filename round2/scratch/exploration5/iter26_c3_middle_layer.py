...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        if bb is not None and ba is not None:
            ...
            mi = 15
            mi_middle = 8   # c3: middle layer size
            bid_middle = int(dynamic_fv) - 3
            ask_middle = int(dynamic_fv) + 3
            if bid_pj <= bid_edge:
                if buy_cap > 0:
                    orders.append(Order(product, bid_edge, buy_cap))
            else:
                bpj = min(mi, buy_cap)
                bmid = min(mi_middle, max(0, buy_cap - bpj))
                be = buy_cap - bpj - bmid
                if be > 0:
                    orders.append(Order(product, bid_edge, be))
                if bmid > 0 and bid_middle < bid_pj:
                    orders.append(Order(product, bid_middle, bmid))
                if bpj > 0:
            ...
            if ask_pj >= ask_edge:
                ...
                spj = min(mi, sell_cap)
                smid = min(mi_middle, max(0, sell_cap - spj))
                se = sell_cap - spj - smid
                if se > 0:
                    orders.append(Order(product, ask_edge, -se))
                if smid > 0 and ask_middle > ask_pj:
                    orders.append(Order(product, ask_middle, -smid))
                if spj > 0:
