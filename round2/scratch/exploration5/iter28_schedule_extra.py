...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0, ts=0):
        ...
        # iter28: schedule-aware extra quote at wall price
        sched_osm = OSM_SCHEDULE.get(ts, [])
        for dr, qty in sched_osm:
            if dr == "BUY" and ba is not None:
                # Scheduled buy-taker coming: post extra ask at wall price (ba)
                extra_sell = min(qty, max(0, LIMITS[product] + pos - sum(abs(o.quantity) for o in orders if o.quantity < 0)))
                if extra_sell > 0:
                    orders.append(Order(product, ba, -extra_sell))
            elif dr == "SELL" and bb is not None:
                extra_buy = min(qty, max(0, LIMITS[product] - pos - sum(o.quantity for o in orders if o.quantity > 0)))
                if extra_buy > 0:
                    orders.append(Order(product, bb, extra_buy))
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0, ts=0):
        ...
        # iter28: schedule-aware extra quote at wall price (PEP)
        sched_pep = PEP_SCHEDULE.get(ts, [])
        for dr, qty in sched_pep:
            if dr == "BUY" and ba is not None:
                extra_sell = min(qty, max(0, limit + pos - sum(abs(o.quantity) for o in orders if o.quantity < 0)))
                if extra_sell > 0:
                    orders.append(Order(product, ba, -extra_sell))
            elif dr == "SELL" and bb is not None:
                extra_buy = min(qty, max(0, limit - pos - sum(o.quantity for o in orders if o.quantity > 0)))
                if extra_buy > 0:
                    orders.append(Order(product, bb, extra_buy))
