...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        if bb is not None and ba is not None:
            ...
            mi = 15
            ladder_size = 8
            # iter27: additional ladder quotes between ask_pj and ask_edge
            bid_l3 = int(dynamic_fv) - 3
            bid_l5 = int(dynamic_fv) - 5
            ask_l3 = int(dynamic_fv) + 3
            ask_l5 = int(dynamic_fv) + 5
            if bid_pj <= bid_edge:
                if buy_cap > 0:
                ...
                bpj = min(mi, buy_cap)
                be = buy_cap - bpj
                b_ladder = min(ladder_size * 2, max(0, be))
                be = be - b_ladder
                if be > 0:
                    orders.append(Order(product, bid_edge, be))
                if b_ladder > 0:
                    orders.append(Order(product, bid_l5, b_ladder // 2))
                    orders.append(Order(product, bid_l3, b_ladder // 2))
                if bpj > 0:
            ...
            if ask_pj >= ask_edge:
                ...
                spj = min(mi, sell_cap)
                se = sell_cap - spj
                s_ladder = min(ladder_size * 2, max(0, se))
                se = se - s_ladder
                if se > 0:
                    orders.append(Order(product, ask_edge, -se))
                if s_ladder > 0:
                    orders.append(Order(product, ask_l3, -(s_ladder // 2)))
                    orders.append(Order(product, ask_l5, -(s_ladder // 2)))
                if spj > 0:
