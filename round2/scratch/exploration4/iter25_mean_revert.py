"""Iter25 — iter23 + OSM mid-regime mean-reversion tilt.

Observed on iter23 baseline (submission 303257): OSM mid spends 52.5%
of ticks >= 10004. During elevated regimes, iter23 has mixed PnL —
gaining in periods that end with reversion, losing in periods that
stay elevated. Total +$1188 over 525 high-regime ticks.

Hypothesis: a tilt that accumulates SHORT during high mid and LONG
during low mid would capture the reversion better.

Mechanism: on OSM, when dynamic_fv >= HIGH_THRESH, reduce buy_cap
(or withdraw bid entirely) and expand sell_cap; when <= LOW_THRESH,
mirror.
...
# iter25: mean-reversion tilt on OSM
OSM_HIGH_THRESH = 10005
OSM_LOW_THRESH = 9997
MR_BUY_SHRINK = 0.3   # on sustained high mid, shrink buy_cap to 30%
MR_SELL_SHRINK = 0.3
...
class Trader:
    def __init__(self):
        ...
        self._tick = 0
        self._osm_high_run = 0   # iter25: consecutive ticks with OSM mid >= HIGH_THRESH
        self._osm_low_run = 0
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        # iter25: update sustained-elevation counters
        if dynamic_fv >= OSM_HIGH_THRESH:
            self._osm_high_run += 1
            self._osm_low_run = 0
        elif dynamic_fv <= OSM_LOW_THRESH:
            self._osm_low_run += 1
            self._osm_high_run = 0
        else:
            self._osm_high_run = 0
            self._osm_low_run = 0
        ...
        # iter25: mean-reversion tilt. Only fires on SUSTAINED elevation (>=5 ticks
        # in a row at >=10005). Short-tilt scales with run length to max shrink.
        MR_RUN_THRESH = 5
        if self._osm_high_run >= MR_RUN_THRESH:
            buy_cap = int(round(buy_cap * MR_BUY_SHRINK))
        elif self._osm_low_run >= MR_RUN_THRESH:
            sell_cap = int(round(sell_cap * MR_SELL_SHRINK))
        ...
        if bb is not None and ba is not None:
            ...
            mi = 15
            # iter25: in sustained high mid, withdraw INNER bid; in sustained low mid, withdraw INNER ask.
            mi_bid = mi
            mi_ask = mi
            if self._osm_high_run >= MR_RUN_THRESH:
                mi_bid = 0   # no inner bid during sustained elevation
            elif self._osm_low_run >= MR_RUN_THRESH:
                mi_ask = 0   # no inner ask during sustained low
            if bid_pj <= bid_edge:
                if buy_cap > 0:
                    orders.append(Order(product, bid_edge, buy_cap))
            else:
                bpj = min(mi_bid, buy_cap)
                be = buy_cap - bpj
                if be > 0:
