...
class Trader:
    def __init__(self):
        ...
        self._tick = 0
        # c2: sustained-regime tracking for OSM size boost
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        # c2: update sustained-regime counters
        if dynamic_fv >= 10005:
            self._osm_high_run += 1
            self._osm_low_run = 0
        elif dynamic_fv <= 9997:
            self._osm_low_run += 1
            self._osm_high_run = 0
        else:
            self._osm_high_run = 0
        ...
        if bb is not None and ba is not None:
            ...
            mi = 15
            # c2: size boost after 5+ consecutive elevated/deflated ticks
            if self._osm_high_run >= 5 or self._osm_low_run >= 5:
                mi = 25
            if bid_pj <= bid_edge:
                if buy_cap > 0:
