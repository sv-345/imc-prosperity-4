"""Iter 18 — iter12 + direct OSM mean-reversion scalp on |mom_5| ≥ 5.

iter17 tried to FILTER fills during +mom (reduce sell size). That worked
marginally in MC (+$100) but not materially on server. This iter tries the
opposite approach: directly TAKE positions when momentum is strong, betting
on reversion.

Signal: ACF(1) of OSM returns = -0.47 (R² 0.22) across 4 slices. Strong
mean reversion: a +$5 move in last 5 ticks predicts a ~-$2.5 move next tick.

Change: when |mom_5| ≥ 5 on OSM (mid moved ≥5 ticks in last 5), enter a
small (5-unit) fade position by crossing the opposite side. If mom > +5,
SELL 5 OSM at best-bid (short-fade). If mom < -5, BUY 5 at best-ask.
The existing MM quotes handle unwinding over the next 50 ticks.

Expected uplift (derived): 5 units × predicted -2.5 reversion × N events.
OSM 5-tick moves of |Δ| ≥ 5 happen ~10% of ticks (per our data). So ~100
events × 5 × 2.5 = $1250/slice. Realistic 30% capture = +$400/slice.

Falsification: if server PnL doesn't lift by ≥ $300/slice, the signal
is mechanistic (bid-ask bounce) rather than tradable reversion.

Parameters:
  MOM_WINDOW   = 5
  MOM_THRESH   = 5      only fade strong moves
  FADE_SIZE    = 5      units to fade per event (small to avoid runaway)
...
# iter18: mean-reversion fade on OSM
MOM_WINDOW = 5
MOM_THRESH = 5
FADE_SIZE = 5
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, mom_5=0.0):
        ...
        # iter18: mean-reversion scalp on strong momentum
        if mom_5 > MOM_THRESH and ba is not None and pos > -LIMITS[product] + FADE_SIZE:
            # Up-move detected → SELL (fade) at best bid
            if bb is not None:
                fill = min(FADE_SIZE, LIMITS[product] + pos)
                if fill > 0:
                    orders.append(Order(product, bb, -fill))
                    pos -= fill
        elif mom_5 < -MOM_THRESH and bb is not None and pos < LIMITS[product] - FADE_SIZE:
            # Down-move detected → BUY (fade) at best ask
            if ba is not None:
                fill = min(FADE_SIZE, LIMITS[product] - pos)
                if fill > 0:
                    orders.append(Order(product, ba, fill))
                    pos += fill
