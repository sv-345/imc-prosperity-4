"""Iter 19 — iter12 + PEP momentum-conditional aggressive cross.

Fill-outcome data (fill_outcomes.py): PEP BUY when 5-tick mom < -2:
  **45 fills, +$6.12/unit realized pnl_50** — best entry condition across
all buckets and products.

PEP BUY when mom ∈ [-2, +2]: 33 fills, **-$0.37/unit** — losing.

Change: track PEP mid history. When mom_5 < -MOM_THRESH and pos < 70,
cross asks up to fv_int + 10 (instead of the default fv_int + 6). This
aggressively accumulates during predicted-good entry conditions.

Expected uplift (data-derived): ~5-10 additional fills per slice at
+$6/unit = +$30-60/slice. Modest. Main hope is that good-condition buys
are more profitable than my average buys.

Falsification: server PnL doesn't lift by ≥ $100/slice vs iter12 $9.4k
baseline.

Parameters:
  MOM_WINDOW      = 5
  MOM_THRESH      = 2
  PEP_AGG_LIMIT   = 10   (vs default 6)
...
# iter19: PEP momentum-conditional fast-cross
MOM_WINDOW = 5
MOM_THRESH = 2
PEP_AGG_LIMIT = 10  # vs default 6
...
class Trader:
    ...
    def run(self, state):
        ...
        # Compute PEP mom_5 from tracked history
        pep_mom_5 = 0.0
        if pep in state.order_depths:
            pep_d = state.order_depths[pep]
            if pep_d.buy_orders and pep_d.sell_orders:
                pep_mid_now = 0.5 * (max(pep_d.buy_orders) + min(pep_d.sell_orders))
                if len(self._pep_mids) >= MOM_WINDOW:
                    pep_mom_5 = pep_mid_now - self._pep_mids[-MOM_WINDOW]
                self._pep_mids.append(pep_mid_now)
                if len(self._pep_mids) > MOM_WINDOW + 1:
                    self._pep_mids = self._pep_mids[-(MOM_WINDOW + 1):]
        ...
        td = json.dumps({
            ...
            "pm": self._pep_mids[-(MOM_WINDOW + 1):] if self._pep_mids else [],
        })
        return result, 0, td
    ...
    def _trade_pepper(self, product, d, pos, mom_5=0.0):
        ...
        if pos < 70:
            # iter19: widen accumulation buy_limit when mom_5 < -MOM_THRESH
            # (fill-outcome data: PEP BUY -mom realized +$6.12/unit).
            if mom_5 < -MOM_THRESH:
                buy_limit = fv_int + PEP_AGG_LIMIT
            else:
                buy_limit = fv_int + 6
            for ap in sorted(d.sell_orders):
