"""Iter 24 — iter23 + signal-boost OSM sell on -sig.

iter23 gates PEP recycle sell size by signal — won $9,867 peak, $9,684
mean. Applying same pattern to OSM MM sells:
  +sig: SHRINK sell_cap (iter22 already did this — mid rising)
  -sig: BOOST sell_cap (mid dropping = good time to sell aggressively)
  0sig: default

Change from iter23: in OSM MM quote block, replace `sell_cap` with
`sell_cap * {0.3 / 1.0 / 1.5}` based on signal.

PEP recycle and v82 structure unchanged from iter23.
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0):
        ...
        if bb is not None and ba is not None:
            ...
            # iter22/24: signal-adjusted caps on OSM MM
            if defend_ask:
                ask_pj = ask_pj + ADVERSE_WIDEN
                sell_cap = int(round(sell_cap * ADVERSE_SHRINK))
            elif take_sig <= -NET_TAKE_THRESH_DEF:
                # iter24: -sig = mid dropping = good sell window, BOOST sell
                sell_cap = int(round(sell_cap * 1.5))
                sell_cap = min(sell_cap, LIMITS[product] + opos)
            if defend_bid:
                bid_pj = bid_pj - ADVERSE_WIDEN
                buy_cap = int(round(buy_cap * ADVERSE_SHRINK))
            elif take_sig >= NET_TAKE_THRESH_DEF:
                # iter24: +sig = mid rising = good buy window, BOOST buy
                buy_cap = int(round(buy_cap * 1.5))
                buy_cap = min(buy_cap, LIMITS[product] - opos)
            bid_edge = fv - edge
            ask_edge = fv + edge
