"""Iter 17 — iter12 + OSM sell-momentum filter (fill-outcome derived).

Fill-outcome analysis (docs/round2_postmortem from fill_outcomes.py) shows:

  OSM SELL fills when mom_5 > +2 (mid rose ≥ 2 in last 5 ticks):
    59 fills, qty 328, Σ pnl50 = +598, pnl50/qty = **+$1.82/unit**

  OSM SELL fills when mom_5 ≤ 0:
    13 fills at -mom: pnl50/qty = **+$7.81/unit**
    86 fills at 0mom: pnl50/qty = **+$5.01/unit**

Selling OSM when mid has just been rising produces ~3× worse realized PnL.
We're quoting asks that get hit by buy-takers during up-moves, right before
the price moves UP further — we sold too cheap.

Change: track recent 5-tick OSM mid (persist via traderData). When
mom_5 > +2, set ask_scale = 0.3 (quote only 30% of normal sell size).
Leave buys untouched. Symmetric bid filter not applied (per data, OSM
buys are less sensitive to mom).

Expected uplift: ~+$342/slice if the 59 +mom sells we didn't take simply
don't fill (we keep the position to sell later at better prices). If the
quotes are still hit at reduced size, uplift is proportionally smaller.

Parameters:
  MOM_WINDOW    = 5          ticks of lookback for momentum
  MOM_THRESHOLD = 2          ticks of mid move to trigger filter
  SELL_SHRINK   = 0.3        shrink factor on OSM ask size when +mom

Falsification: if server PnL doesn't move by ≥ $200/slice vs iter12's
$9.4k/slice baseline, hypothesis is wrong — the fill-outcome correlation
doesn't translate to real-time filter ability.
...
# iter17: OSM momentum-based sell filter.
MOM_WINDOW = 5
MOM_THRESHOLD = 2
SELL_SHRINK = 0.3
...
class Trader:
    def __init__(self):
        ...
        self._tick = 0
        # iter17: OSM mid history (most recent first)
        self._osm_mids = []
    ...
    def run(self, state):
        ...
        # Update OSM mid history (track last MOM_WINDOW+1 mids)
        osm = "ASH_COATED_OSMIUM"
        osm_mom_5 = 0.0
        if osm in state.order_depths:
            d = state.order_depths[osm]
            if d.buy_orders and d.sell_orders:
                osm_mid_now = 0.5 * (max(d.buy_orders) + min(d.sell_orders))
                # compute mom_5 against the oldest stored mid
                if len(self._osm_mids) >= MOM_WINDOW:
                    osm_mom_5 = osm_mid_now - self._osm_mids[-MOM_WINDOW]
                self._osm_mids.append(osm_mid_now)
                if len(self._osm_mids) > MOM_WINDOW + 1:
                    self._osm_mids = self._osm_mids[-(MOM_WINDOW + 1):]
        ...
        td = json.dumps({
            ...
            "om": self._osm_mids[-(MOM_WINDOW + 1):] if self._osm_mids else [],
        })
        return result, 0, td

    def _trade_osmium(self, product, d, pos, mom_5=0.0):
        fv = OSM_FV
        orders = []
        opos = pos
        # iter17: shrink ask size during +mom (avoid selling into up-moves).
        sell_shrink = SELL_SHRINK if mom_5 > MOM_THRESHOLD else 1.0
        ...
        sell_cap = max(0, int(round((LIMITS[product] + opos - sell_swept) * sell_shrink)))
