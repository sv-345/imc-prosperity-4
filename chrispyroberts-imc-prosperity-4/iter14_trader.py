"""Iter 14 — iter12 + widen PEP buy_limit to fv_int+10 (sweep inner + wall asks).

iter13 (CF3: tighten OSM take) regressed slightly on server — reverting to iter12.
This iter keeps iter12's OSM and changes only PEP: widen the accumulation-phase
buy_limit from fv_int+6 to fv_int+10. The +6 threshold stops just below the
PEP inner ask at fair+7; +10 reaches the wall at fair+10.

Rationale:
  - iter12 server shows PEP reaches ~80 long eventually but recycling is
    modest ($300-400 extra over pure drift hold). Faster accumulation (sweep
    inner + wall in 1-2 ticks instead of arrival-rate-limited) = more time
    at +80 to capture drift.
  - R1 post-mortem CF2 (PEP fast-ramp) estimated +$150-300 hindsight realistic
    on 10k ticks; 1k-tick scaled ≈ +$50-150. Not enormous but evidence-backed.
  - Cost: paying +3-4 ticks per wall-ask fill. Offset by ~$96/unit drift
    capture over the remaining hold time.
...
class Trader:
    ...
    def _trade_pepper(self, product, d, pos):
        ...
        if pos < 70:
            # iter14: sweep through inner (fair+7) and wall (fair+10) asks.
            buy_limit = fv_int + 10
            for ap in sorted(d.sell_orders):
