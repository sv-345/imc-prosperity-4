...
# iter27: phase-3 widening. Tape distribution is bimodal at mid±5+ on OSM with
# near-zero volume inside ±3, so pushing quotes from mid±1 to mid±K captures
# almost the same volume at K× edge. Tunable via env var for sweep harness.
import os as _os
OSM_PHASE3_K_WINDOW = int(_os.environ.get("OSM_PHASE3_K_WINDOW", "3"))
OSM_PHASE3_K_OUTSIDE = int(_os.environ.get("OSM_PHASE3_K_OUTSIDE", "1"))
PEP_PHASE3_K_WINDOW = int(_os.environ.get("PEP_PHASE3_K_WINDOW", "1"))
PEP_PHASE3_K_OUTSIDE = int(_os.environ.get("PEP_PHASE3_K_OUTSIDE", "1"))
# Server-scale windows. On training (1M ts/day), these cover 2.78% of each day.
...
class Trader:
    ...
    def run(self, state):
        ...
        osm_k = OSM_PHASE3_K_WINDOW if in_win else OSM_PHASE3_K_OUTSIDE
        pep_k = PEP_PHASE3_K_WINDOW if in_win else PEP_PHASE3_K_OUTSIDE
    ...
    def _trade_osmium(self, product, d, pos, take_sig=0, osm_k=1):
        fv = OSM_FV
        orders = []
        opos = pos
        ...
        if bb is not None and ba is not None:
            # iter27: widen quotes to mid±OSM_PHASE3_K to harvest bimodal tape
            # (95% of OSM tape is at mid±5+). Quote may not cross book.
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0, pep_k=1):
        ...
        if pos < 70:
            ...
            if buy_cap > 0 and bb is not None:
                bid_price = min(bb + 1, fv_int - pep_k)
                if defend_bid:
            ...
            if sell_qty > 0:
                ask_price = max(ba - 1, fv_int + pep_k) if ba else fv_int + 7
