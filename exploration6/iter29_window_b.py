...
OSM_PHASE3_K = 5
PEP_PHASE3_K = 1
_WINDOWS_B = ((34300, 48100), (76100, 90100))
def _in_window_b(ts):
    for lo, hi in _WINDOWS_B:
...
class Trader:
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0):
        ...
        if pos < 70:
            ...
            if sell_qty > 0:
                # iter29 variant B: within windows, quote ask at fv_int+3 (chase drift).
                if _in_window_b(getattr(self, "_current_ts", 0)):
                    ask_price = fv_int + 3
                else:
