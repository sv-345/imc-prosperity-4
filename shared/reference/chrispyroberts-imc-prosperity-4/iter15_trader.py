"""Iter 15 — iter12 (v82 base) + top-of-book imbalance inventory lean.

Feature analysis on 4 v82-family submissions showed top-of-book imbalance
predicts fwd-1 mid move with slope +5, R² 0.30-0.33 on BOTH products,
across all 4 slices. v82 uses none of this.

Change: compute imbalance = (bv1 - av1) / (bv1 + av1). When |imbalance| ≥
IMB_THRESHOLD, scale quote sizes by (1 + IMB_SIZE_K × imbalance) to tilt
inventory in the signal direction.

Falsification: server uplift ≥ $1/tick vs iter12 $9.4/tick, or hypothesis
wrong. See docs/round2_alpha_hypothesis.md.

Parameters (adds 1 tuned: IMB_THRESHOLD):
  IMB_THRESHOLD = 0.4          capture ~25 % of strongest-signal ticks
  IMB_SIZE_K    = 0.75         tilt coefficient
...
# Imbalance lean — derived from R² 0.33 fwd-1 regression on OSM & PEP.
IMB_THRESHOLD = 0.4
IMB_SIZE_K = 0.75


def top_imbalance(d):
    """Top-of-book imbalance ∈ [-1, +1]. Returns 0 if book is empty."""
...
def imbalance_size_factor(imb):
    """Return (bid_scale, ask_scale) in [0.25, 1.75].
    Positive imbalance = more bid vol than ask vol = signal mid will move UP.
    We want to be LONGER → scale up bid qty, scale down ask qty.
    Only applied when |imb| ≥ IMB_THRESHOLD."""
    if abs(imb) < IMB_THRESHOLD:
        return 1.0, 1.0
    tilt = IMB_SIZE_K * imb  # ∈ [-0.75, +0.75]
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos):
        ...
        # iter15: imbalance lean — scale quote sizes by signal direction.
        imb = top_imbalance(d)
        bid_scale, ask_scale = imbalance_size_factor(imb)
        buy_cap = int(min(LIMITS[product] - opos, round(buy_cap * bid_scale)))
        sell_cap = int(min(LIMITS[product] + opos, round(sell_cap * ask_scale)))
