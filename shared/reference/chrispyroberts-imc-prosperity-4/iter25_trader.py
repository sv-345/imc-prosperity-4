"""Iter 25 — iter23 + tighter PEP passive bid (harvest uninformed flow).

Fingerprint analysis (docs/round2_take_prediction.md): PEP bot takes show
avg directional mid-move ~0 after accounting for drift — suggesting
UNINFORMED flow. If so, tighter quoting harvests their fills without
adverse selection.

Change from iter23: PEP bb+1 → bb+2 in both accumulation-phase fallback
bid and recycle-phase bid. Pay 1 extra tick per fill, gain queue priority
over any bot-3 passive bid at fv-3/-4.

Expected: ~10 additional PEP fills per slice × 5 qty × net $0 (paying
$1 spread, capturing $1 of drift over 10 ticks) to maybe +$1/unit net.
Target uplift +$50-100/slice.

Original iter23 docstring:
---
iter22 + signal-gated PEP recycle size.
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0):
        ...
        if pos < 70:
            ...
            if remaining > 0 and bb is not None:
                # iter25: bb+2 for tighter priority (was bb+1)
                orders.append(Order(product, bb + 2, remaining))
        else:
            # Recycle: buy any remaining capacity inside bb, sell 8 at ba-1
            buy_cap = limit - pos
            if buy_cap > 0 and bb is not None:
                # iter25: bb+2 tighter (was bb+1)
                bid_price = min(bb + 2, fv_int - 1)
                if defend_bid:
