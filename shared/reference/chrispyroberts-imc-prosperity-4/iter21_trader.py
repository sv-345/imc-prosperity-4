"""Iter 21 — iter20 + direct TAKE on strong bot-take signal.

iter20 price-shift didn't convert the z=19.5 signal. Hypothesis: by the
time I see market_trades at tick T, the T-1 take's move has already
happened; price-shifting my next-tick quote catches a tiny residual.

iter21 tries direct position-taking. When net bot-take signal is strong
(|sig| ≥ STRONG_THRESH = 5 qty units), CROSS the book for N units
immediately. Bet: clustered takes continue, subsequent flow makes our
crossed entry profitable.

Applied to PEP only (OSM signal z=+2.76 too small to justify spread cost;
PEP z=+19.5 justifies).

Change from iter20:
  In _trade_pepper, after the normal take-below-fv loop, if take_sig ≥
  STRONG_THRESH and pos < 75, cross ap1 for CROSS_SIZE=5 units.

Parameters:
  STRONG_THRESH = 5 (unit-qty net of takes last tick)
  CROSS_SIZE    = 5 (units to cross on signal)

Expected: 5-10 strong PEP signal events per slice × 5 units × $3 net
post-spread (including drift) = +$100-250/slice.

Falsification: server PnL doesn't move ≥ +$100/slice — signal decays
faster than my spread cost.

Original iter20 docstring:
---
iter12 + bot-take direction inventory lean.
...
# iter21: direct PEP take-on-strong-signal
STRONG_THRESH = 5
CROSS_SIZE = 5
...
class Trader:
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0):
        ...
        # iter21: direct cross on strong PEP buy-take signal (z=19.5).
        # Bet on clustered follow-through after a large recent buy-take.
        if take_sig >= STRONG_THRESH and pos < 75 and ba is not None:
            cross_fill = min(CROSS_SIZE, limit - pos, -d.sell_orders.get(ba, 0))
            if cross_fill > 0:
                orders.append(Order(product, ba, cross_fill))
                pos += cross_fill
