"""Iter 26 — iter23 architecture change: never unwind PEP.

iter23 wins via signal-gated PEP recycle size. iter26 tests Option 4:
remove the recycle branch entirely. Just accumulate to +80 then HOLD to
end of slice. Captures pure drift income (80 × $100 = $8,000 / slice).

Test: does pure-hold beat recycle-with-signal-gate?
Data: iter23 PEP mean $7,752 across 3 runs.
Pure-hold ceiling: 80 × (mid_end - mid_avg_buy) - accumulation cost.
Estimated pure-hold: ~$7,500-8,000 depending on accumulation speed.

If iter26 PEP > iter23 PEP, recycling is net-negative and we keep iter26.
If iter26 PEP ≤ iter23 PEP, recycling was net-positive and iter23 wins.

OSM unchanged from iter23.
...
class Trader:
    ...
    def _trade_pepper(self, product, d, pos, take_sig=0):
        ...
        if pos < 70:
            ...
            # iter26: NO recycle sell — pure hold to capture full drift.
