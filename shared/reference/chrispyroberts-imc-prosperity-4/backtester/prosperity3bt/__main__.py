...
def monte_carlo_cli(
    ...
    round_num: Annotated[int, Option("--round", "-r", help="Prosperity round to simulate (0 = tutorial EMERALDS/TOMATOES, 2 = ASH_COATED_OSMIUM/INTARIAN_PEPPER_ROOT).")] = 0,
) -> None:
    if no_out:
    ...
    if round_num not in (0, 2):
        print(f"Error: --round {round_num} is not supported (valid: 0, 2)")
        sys.exit(1)
    ...
    # Per-product mean PnL — useful for diagnosing which product is leaking.
    for product, stats in sorted(dashboard.get("products", {}).items()):
        pnl_stats = stats.get("pnl", {})
        if pnl_stats.get("count", 0) == 0:
            continue
        # Skip legacy EMERALDS/TOMATOES rows when running R2 (they're 0-filled).
