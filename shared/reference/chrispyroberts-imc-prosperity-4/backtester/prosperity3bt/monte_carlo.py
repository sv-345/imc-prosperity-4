...
def resolve_actual_dir(data_root: Optional[Path], round_num: int = 0) -> Path:
    round_dirname = f"round{round_num}"
    if data_root is None:
        return project_root() / "data" / round_dirname

    if data_root.name == round_dirname:
        return data_root

    nested = data_root / round_dirname
    if nested.is_dir():
        return nested
...
def _parse_product_stats(raw: str) -> dict[str, dict[str, float]]:
    """Parse the `product_stats_json` column written by the Rust simulator.

    Returns a dict keyed by product name with pnl/cash/position/slope_per_step/
    r_squared fields. Missing or empty strings return `{}`.
    """
    if not raw:
        return {}
    try:
    ...
    for product, stats in data.items():
        result[product] = {
            ...
            "position": int(stats.get("position", 0)),
            "slopePerStep": float(stats.get("slope_per_step", 0.0)),
        ...
        parsed.append(
            {
                ...
                "productStats": _parse_product_stats(row.get("product_stats_json", "")),
            }
        )
    return parsed
...
def load_sample_session(session_dir: Path, round_num: int = 0) -> dict[str, Any]:
    round_dir = session_dir / f"round{round_num}"
    if not round_dir.is_dir():
        # Back-compat: some older sample dirs may use the bare "round0" name.
    ...
    day_files = sorted(
        ...
        for path in round_dir.glob(f"trace_round_{round_num}_day_*.csv")
    )

    for day_index, day in enumerate(day_files):
        trace_rows = read_csv_dicts(round_dir / f"trace_round_{round_num}_day_{day}.csv", ";")
        price_rows = read_csv_dicts(round_dir / f"prices_round_{round_num}_day_{day}.csv", ";")
...
def write_sample_path_sidecars(output_dir: Path, sample_session_dirs: list[Path], round_num: int = 0) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for session_dir in sample_session_dirs:
        sample = load_sample_session(session_dir, round_num=round_num)
...
def write_static_chart_svgs(output_dir: Path, sampled_paths: list[dict[str, Any]]) -> dict[str, list[dict[str, str]]]:
    ...
    # Build chart specs from whatever products appear in the first sample
    # path. Closes over `product` via default-arg binding so each lambda
...
def _product_series(
    ...
    """Extract a per-product field. Prefers `productStats[product][field]`
    (written by the R2-aware Rust simulator). Falls back to `row[legacy_field]`
    for compatibility with older CSVs that only had emerald_*/tomato_* columns.
    """
    out = []
    for row in rows:
        stats = row.get("productStats", {})
        if product in stats and field in stats[product]:
            out.append(float(stats[product][field]))
        elif legacy_field is not None and legacy_field in row:
            out.append(float(row[legacy_field]))
        else:
            out.append(0.0)
    return out
...
def build_dashboard(
    ...
    # Resolve the round's product list from the first session's productStats.
    # For R0 this is ["EMERALDS", "TOMATOES"]; for R2 it's OSM + PEP.
    if session_rows and session_rows[0].get("productStats"):
        product_list = sorted(session_rows[0]["productStats"].keys())
    else:
        product_list = ["EMERALDS", "TOMATOES"]

    # Keep the emerald/tomato named lists for the legacy (R0) dashboard fields.
    ...
    sample_path_refs, sampled_paths = write_sample_path_sidecars(output_dir, sample_session_dirs, round_num=round_num) if sample_session_dirs else ([], [])
    ...
    return {
        ...
        "products": {
            **{
                # Legacy R0 EMERALDS / TOMATOES keys, kept for viz back-compat.
                "EMERALDS": {
            ...
            # Additive generic per-product keys (populated from productStats).
            **{
                product: {
                    "pnl": summarize_distribution(_product_series(session_rows, product, "pnl")),
                    "finalPosition": summarize_distribution(_product_series(session_rows, product, "position")),
                    "cash": summarize_distribution(_product_series(session_rows, product, "cash")),
                }
        ...
        "histograms": {
            ...
            **{
                f"{product}Pnl": histogram(_product_series(session_rows, product, "pnl"))
                for product in product_list
            },
        },
...
def run_rust_monte_carlo(
    ...
    actual_dir = resolve_actual_dir(data_root, round_num)
