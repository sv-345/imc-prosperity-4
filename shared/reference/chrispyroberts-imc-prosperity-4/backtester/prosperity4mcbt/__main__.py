...
def cli(
    ...
    round_num: Annotated[
        int,
        Option("--round", "-r", help="Prosperity round to simulate (0 = tutorial EMERALDS/TOMATOES, 2 = ASH_COATED_OSMIUM/INTARIAN_PEPPER_ROOT, 3 = HYDROGEL_PACK/VELVETFRUIT_EXTRACT/VEV vouchers)."),
    ] = 0,
    version: Annotated[
        bool,
    ...
    if round_num not in (0, 2, 3):
        print(f"Error: --round {round_num} is not supported (valid: 0, 2, 3)")
        raise SystemExit(1)
    ...
    for product, stats in sorted(dashboard.get("products", {}).items()):
        pnl_stats = stats.get("pnl", {})
        if pnl_stats.get("count", 0) == 0:
            continue
        if round_num == 2 and product in ("EMERALDS", "TOMATOES"):
            continue
        if round_num == 0 and product not in ("EMERALDS", "TOMATOES"):
            continue
        # R3: only show R3 products
        r3_products = {"HYDROGEL_PACK", "VELVETFRUIT_EXTRACT"} | {f"VEV_{k}" for k in (4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500, 6000, 6500)}
        if round_num == 3 and product not in r3_products:
            continue
        print(
            f"  {product}: mean={pnl_stats['mean']:,.2f}  "
            f"std={pnl_stats['std']:,.2f}  "
            f"p05={pnl_stats['p05']:,.2f}  "
            f"pos>0 rate={pnl_stats.get('positiveRate', 0):.2%}"
        )
