...
Round 2 adds two products to the simulator:

- `ASH_COATED_OSMIUM` (OSM) — stationary around fair value `10001`
- `INTARIAN_PEPPER_ROOT` (PEP) — deterministic linear drift `+0.10/tick`,
  with per-day starting fairs of `11000 / 12000 / 13000` on days
  `-1 / 0 / 1`

Round 2 data lives in [`data/round2/`](data/round2). The calibration
pipeline is in [`scripts/round2_calibration/calibrate_round2.py`](scripts/round2_calibration/calibrate_round2.py);
it writes [`docs/round2_params.json`](docs/round2_params.json) and
[`docs/round2_calibration_summary.txt`](docs/round2_calibration_summary.txt).

### What Works Right Now (Round 2)

- 3-day synthetic CSV generation (days `-1`, `0`, `1`) for OSM + PEP
- Calibrated book-placement model: outer wall, inner wall, near-mid
  "bot-3" noise quotes with product-specific passive/aggressive split
- Calibrated taker flow: IID Bernoulli arrivals, 50/50 side mix, uniform
  quantity in the measured support (`OSM: U(2,10)`, `PEP: U(3,8)`)

### What's Not Wired Yet (Round 2)

- The Monte Carlo **strategy runner**. Session accounting in
  `rust_simulator/src/main.rs` and the dashboard builder still index on
  `emerald_pnl` / `tomato_pnl` columns, so `prosperity4mcbt --round 2 <trader>.py`
  currently exits with a message directing you to the raw data-generation
  binary. See [`docs/round2_model.md`](docs/round2_model.md) §4 for the
  remaining refactor needed.
- The visualizer's hard-coded `EMERALDS` / `TOMATOES` labels. Extending
  them is a cosmetic follow-up once the dashboard bundle carries OSM/PEP
  stats.

### Generate Round 2 Synthetic Data

```bash
cd rust_simulator
cargo build --release
./target/release/rust_simulator \
    --round 2 --fv-mode simulate --trade-mode simulate \
    --output ../tmp/round2_gen
ls ../tmp/round2_gen/round2/
# prices_round_2_day_{-1,0,1}.csv  trades_round_2_day_{-1,0,1}.csv
```

### Parameter Disclosure

Every Round 2 literal in the Rust simulator has a comment pointing to its
key in `docs/round2_params.json`. The most load-bearing values:

| Product | Fair | Walls (bid / ask) | Inners (bid / ask) | Wall vol | Inner vol | Taker rate | Taker qty |
|---|---:|---:|---:|---:|---:|---:|---:|
| OSM | 10001 | -10 / +9 | -8 / +8 | U(20,30) | U(10,15) | 4.60% | U(2,10) |
| PEP | day-start + 0.10·t | -10 / +10 | -6 / +7 | U(15,25) | U(8,12) | 3.31% | U(3,8) |

### How It Differs From Round 0

- Round 0 (EMERALDS/TOMATOES) walls are ±10 / ±8. Round 2 OSM walls are
  asymmetric at -10 / +9 (the ask is one tick tighter than R1's +11).
- Round 0 TOMATOES uses a latent Gaussian random walk; Round 2 PEP is
  deterministic (`F_t = day_start + 0.10·t`).
- Round 0 has 2 training days; Round 2 has 3 (`-1`, `0`, `1`).
- Round 2 bot-3 presence is asymmetric across bid/ask for PEP (54% bid
  vs 7.5% ask), which reflects the upward drift pulling fair through
  resting asks each tick.

### Example Trader

[`example_trader_round2.py`](example_trader_round2.py) is a reasonable
(not optimal) market-maker that quotes one tick inside the best visible
bid/ask on both products. Use it as a smoke-test for any R2-aware
toolchain.
