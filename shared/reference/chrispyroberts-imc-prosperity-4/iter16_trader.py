"""Iter 16 — iter12 + PRICE-shift imbalance lean on OSM.

iter15 size-scaling didn't convert R²=0.33 imbalance signal to PnL (fills
happen at best quote prices regardless of my size, as long as size ≥ typical
taker qty of 10). Price-shift approach instead.

Change from iter12 (v82 base): in _trade_osmium, when |imbalance| ≥
IMB_THRESHOLD, shift bid_pj and ask_pj by round(IMB_PRICE_K × imb) ticks in
the signal direction. This tilts the quote stack UP on positive imbalance
→ passive bid becomes more aggressive (higher price, catches sell-takers at
better-for-us price) and passive ask retreats (less likely to fill into
predicted up-move). Symmetric for negative imbalance.

Take logic, wide-snipe layer, and PEP logic unchanged. v82's fv-1/fv+1
caps on bid_pj/ask_pj are applied AFTER the shift so we never cross fv.

Parameters (adds 1 tuned vs iter12):
  IMB_THRESHOLD = 0.4    — capture ~25% of ticks (|imb| ≥ 0.4)
  IMB_PRICE_K   = 4      — shift up to ±2 ticks at |imb|=0.5
...
# iter16: imbalance price-shift (feature R²=0.33 on fwd-1 OSM mid)
IMB_THRESHOLD = 0.4
IMB_PRICE_K = 4
...
def imbalance_shift(d):
    imb = top_imbalance(d)
    if abs(imb) < IMB_THRESHOLD:
        return 0
    return int(round(IMB_PRICE_K * imb))
...
class Trader:
    ...
    def _trade_osmium(self, product, d, pos):
        ...
        if bb is not None and ba is not None:
            # iter16: shift passive quote prices by imbalance signal.
            shift = imbalance_shift(d)
            # Positive shift → bid_pj higher, ask_pj higher (lean long).
            bid_pj = min(bb + 1 + shift, fv - 1)
            ask_pj = max(ba - 1 + shift, fv + 1)
            # Note: shifting ask UP makes it less likely to fill (good on +imb).
            # Shifting bid UP makes it more aggressive (good on +imb).
            bid_edge = fv - edge
            ask_edge = fv + edge
